...
def get_todays_races():
    ...
    for link in soup.select("a.rc-meeting__race"):
...
def get_race_data(race_url, race_time):
    ...
    soup = BeautifulSoup(r.text, "html.parser")
    data = []
    # Updated selectors for runners & odds
    for row in soup.select("div.runner-row"):
        horse_tag = row.select_one(".runner-name a")
        ...
        try:
            ...
            continue
        data.append({"Horse": horse, "Odds_TF": odds, "Race Time": race_time})
