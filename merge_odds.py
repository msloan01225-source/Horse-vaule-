...
def test_url(url):
    """Fetch a URL and log the first part of HTML for debugging."""
    try:
        r = requests.get(url, headers={"User-Agent": "Mozilla/5.0"})
        r.raise_for_status()
        snippet = r.text[:300].replace("\n", " ").replace("\r", " ")
        log(f"DEBUG HTML from {url}: {snippet}")
    except Exception as e:
        log(f"DEBUG ERROR fetching {url}: {e}")

def safe_scraper(name, func, sample_url=None):
    try:
        log(f"Starting {name} scraper...")
        df = func()
        log(f"{name} scraper finished – {len(df)} rows found.")
        if df.empty and sample_url:
            log(f"{name} returned 0 rows. Testing sample page...")
            test_url(sample_url)
        return df
    except Exception as e:
...
def calculate_countdown(row):
    ...
    tz_map = {"UK": "Europe/London", "AUS": "Australia/Sydney", "USA": "US/Eastern"}
    tz = pytz.timezone(tz_map.get(row["Country"], "UTC"))
...
def merge_best_prices():
    rp_df = safe_scraper("Racing Post", get_rp_data, "https://www.racingpost.com/racecards/time-order/")
    tf_df = safe_scraper("Timeform", get_tf_data, "https://www.timeform.com/horse-racing/racecards")
    aus_df = safe_scraper("Australia", get_aus_data, "https://www.racing.com/racecards")
    usa_df = safe_scraper("USA", get_usa_data, "https://www.equibase.com")
    ...
    if not rp_df.empty and not tf_df.empty:
        ...
        log("⚠ Skipping UK merge – missing Racing Post or Timeform data.")
