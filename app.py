...
def generate_mock_data(n=40):
    ...
    countries = [
        "UK" if c in ("Ascot", "York", "Cheltenham") else "USA"
        for c in courses
    ]
...
if selected == "Overview":
    ...
    st.write(
        "Get the edge over bookmakers with cutting-edge predictive value models."
    )
...
# ---- HORSE RACING PAGE ----
    ...
    with col2:
        bookie = st.selectbox(
            "Select Bookmaker", ["All", "SkyBet", "Bet365", "Betfair"]
        )
    with col3:
        courses = df["Course"].unique().tolist()
        course_filter = st.multiselect(
            "Select Courses", courses, default=courses
        )
    ...
    win_df = filtered.sort_values(
        "BetEdge Win %", ascending=False
    ).reset_index(drop=True)
    place_df = filtered.sort_values(
        "BetEdge Place %", ascending=False
    ).reset_index(drop=True)
    ...
    if filtered.empty:
        ...
        if view_mode == "📊 Charts":
            ...
            st.bar_chart(
                place_df.head(20).set_index("Horse")["BetEdge Place %"]
            )
        else:
...
# ---- EDGEBRAIN PAGE ----
    ...
    st.dataframe(
        df[["Horse", "Course", "Odds", "BetEdge Win %", "Risk"]],
