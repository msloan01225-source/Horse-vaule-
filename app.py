...
def load_data(day):
    ...
    # Simulated odds & probabilities
    df["Best Odds"] = np.random.uniform(2, 6, len(df)).round(2)
    df["Predicted Win %"] = (1 / df["Best Odds"] * 100).round(1)
    df["Predicted Place %"] = (df["Predicted Win %"] * 0.6).round(1)

    # BetEdge algorithm (simple for now: blend values & predictions)
...
if df.empty:
    ...
    st.dataframe(df_win[["Race", "Horse", "Best Odds", "Win_Value", "Predicted Win %"]]
    ...
    st.dataframe(df_place[["Race", "Horse", "Best Odds", "Place_Value", "Predicted Place %"]]
                 .style.applymap(color_val, subset=["Place_Value"]))

    # BetEdge Win Rankings
    st.subheader("🔥 BetEdge Win Rankings")
    df_be_win = df.sort_values(by="BetEdge Win %", ascending=False).reset_index(drop=True)
    st.dataframe(df_be_win[["Race", "Horse", "Best Odds", "BetEdge Win %"]]
                 .style.applymap(color_val, subset=["BetEdge Win %"]))

    # BetEdge Place Rankings
    st.subheader("🔥 BetEdge Place Rankings")
    df_be_place = df.sort_values(by="BetEdge Place %", ascending=False).reset_index(drop=True)
    st.dataframe(df_be_place[["Race", "Horse", "Best Odds", "BetEdge Place %"]]
