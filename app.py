# app.py — EdgeBet (Betfair live odds + Racing API fallback + EdgeBrain v1 + EdgeBrain+ history)
...
:root {
  --brand: #00bfa6;
  --brand-soft: rgba(0, 191, 166, 0.15);
  --text: #f2f2f2;
  --muted: #a0a0a0;
  --border: rgba(255,255,255,0.1);
  --sticky-bg: rgba(17, 17, 17, 0.95);
}
body { background-color: #111111; color: var(--text); }
...
def take_cols(df: pd.DataFrame, wanted: list[str]) -> pd.DataFrame:
    out = df.copy()
    for c in wanted:
...
def build_live_df_betfair(day="today") -> pd.DataFrame:
    ...
    df["Odds (SP proj)"] = np.nan
    df["Odds_Place_EX"] = np.nan
    df["MinsToOff"] = np.nan
...
def fetch_racecards_basic(day="today"):
    ...
    if not user or not pwd:
        return False, None, None, "No Racing API credentials."
...
def build_live_df_racingapi(payload: dict) -> pd.DataFrame:
    ...
    for col in ["EventId","Odds (SP proj)","Odds_Place_EX","MinsToOff"]:
        df[col] = np.nan
    return df.sort_values("BetEdge Win %", ascending=False).reset_index(drop=True)
...
def add_edgebrain_features(df: pd.DataFrame) -> pd.DataFrame:
    eb = df.copy()
    eb["imp_ex"] = 1 / eb["Odds"]
    eb["imp_sp"] = 1 / eb["Odds"].replace(0, np.nan)
    eb["field_size"] = eb.groupby("EventId")["Horse"].transform("count")
    eb["fav_rank"] = eb.groupby("EventId")["Odds"].rank(method="min", ascending=True)
    eb["gap_to_fav"] = eb.groupby("EventId")["Odds"].transform(lambda x: x - x.min())
    eb["p_blend"] = (eb["imp_ex"] + eb["imp_sp"]) / 2
    return eb
...
def edgebrain_softmax(df_feat: pd.DataFrame) -> pd.Series:
    ...
    df_feat["lin"] = lin
    return df_feat.groupby("EventId", group_keys=False).apply(_softmax_group)
...
# ------------------ EDGE BRAIN+ (history-enriched) ------------------
def get_history_data():
    """Mock history provider — replace with real DB/API later."""
    np.random.seed(123)
    data = {
        "Horse": [],
        "hist_win_pct": [],
    ...
    return pd.DataFrame(data)  # currently empty, will be merged safely
...
def edgebrain_plus_score(df: pd.DataFrame) -> pd.Series:
    """EdgeBrain v1 logic + historical features."""
    feats = add_edgebrain_features(df)
    feats["hist_factor"] = (
...
if sel == "Overview":
    ...
    if df.empty:
        ...
        if view == "📊 Charts":
            ...
            wanted = ["Country","Course","Time","Horse","Odds","BetEdge Win %","BetEdge Place %","Source"]
            st.dataframe(take_cols(df, wanted), use_container_width=True)
    ...
    with top_cols[1]:
        model_view = st.radio(
            ...
            horizontal=True,
            help="Compare models or focus on one."
        )
    ...
    if df.empty:
        ...
        df["Risk_v1"] = np.where(df["EdgeBrain Win %"] >= 25, "✅",
                          np.where(df["EdgeBrain Win %"] >= 15, "⚠️", "❌"))
        df["Risk+"]   = np.where(df["EdgeBrain+ Win %"] >= 25, "✅",
                          np.where(df["EdgeBrain+ Win %"] >= 15, "⚠️", "❌"))
        ...
        if model_view == "EdgeBrain v1":
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.caption("EdgeBrain v1 – Top 20")
                st.bar_chart(df.sort_values("EdgeBrain Win %", ascending=False)
                               .head(20).set_index("Horse")["EdgeBrain Win %"])
            with c2:
                st.caption("EdgeBrain+ – Top 20")
                st.bar_chart(df.sort_values("EdgeBrain+ Win %", ascending=False)
                               .head(20).set_index("Horse")["EdgeBrain+ Win %"])

        st.subheader("📋 Predictions table")
        if model_view == "EdgeBrain v1":
            wanted = ["Country","Course","Time","Horse","Odds","EdgeBrain Win %","Risk_v1","Source"]
            st.dataframe(take_cols(df, wanted), use_container_width=True)
        elif model_view == "EdgeBrain+ (history)":
            wanted = ["Country","Course","Time","Horse","Odds","EdgeBrain+ Win %","Risk+","Source"]
            st.dataframe(take_cols(df, wanted), use_container_width=True)
        else:
            def highlight_row(row):
                ...
                if row["EdgeBrain+ Win %"] > row["EdgeBrain Win %"]:
                    return ["background-color: rgba(0,191,166,0.20)"] * len(row)
                elif row["EdgeBrain Win %"] > row["EdgeBrain+ Win %"]:
                    return ["background-color: rgba(255,215,0,0.20)"] * len(row)
                return [""] * len(row)

            wanted = ["Country","Course","Time","Horse","Odds",
                      "EdgeBrain Win %","Risk_v1",
                      "EdgeBrain+ Win %","Risk+",
                      "Source"]
            styled_df = take_cols(df, wanted).style.apply(highlight_row, axis=1)
