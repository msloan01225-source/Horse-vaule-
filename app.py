...
def load_data(day="today"):
    ...
    df = pd.concat([rp, tf], ignore_index=True).drop_duplicates(subset=["Horse","Course","Time"])
    if df.empty:
        return df

    # assign country (all UK for now)
    df["Country"] = "UK"

    # ─── MOCK ODDS & VALUE (swap in your real feed later) ─────────────────────
...
if selected=="Overview":
    ...
    df        = load_data(day)
    country   = st.selectbox("Country", ["All","UK"], index=1)
    ...
    mi,ma     = (int(df["BetEdge Win %"].min()), int(df["BetEdge Win %"].max())) if not df.empty else (0,0)
    ...
    st.markdown("""
      ...
      - 🔎 Risk Filters  
    """)
    st.video("https://www.youtube.com/watch?v=2Xc9gXyf2G4")  # your hosted intro
