# edgebet_app.py — EdgeBet Ultra (Matrix Edition, EdgeBrain v3 + Green→Red Charts + Race Tape)
# ---------------------------------------------------------------------------------------------
# • EdgeBrain v3 Predictor (Top 3 per race)
# • Charts colour‑coded green (best) → red (worst): gradient bars, EV vs Score scatter, heatmap,
#   sparklines with EV glow
# • NEW: Race Tape (per-race EV heat‑ribbons + odds trend) on Predictor + compact ribbon on Overview
# • Removed: Dutching Lab, Race Hub
# ---------------------------------------------------------------------------------------------
...
if "price_hist" not in st.session_state:
    st.session_state.price_hist = defaultdict(lambda: deque(maxlen= ninety := 90))  # 90 pts per runner
...
# ------------------ DESIGN SYSTEM ------------------
PALETTE = {
    "neon":   "#00FF6A", "neon_dim":"#00cc56",
    "purple": "#7C3AED", "white":  "#EDEDED",
    "red":    "#EF4444", "amber":  "#F59E0B",
    "ink":    "#0A0F0A", "ink_2":  "#0D1512",
    "grid":   "#134E4A", "teal":   "#00BFA6", "green":  "#16A34A"
}
...
def _altair_theme():
    return {"config":{
        ...
        "axis":{"labelColor":"var(--c-text)","titleColor":"var(--c-text)","domainColor":"var(--c-axis)","tickColor":"var(--c-axis)","gridColor":"rgba(0,255,106,0.08)"},
...
def _odds_vol(series: list[float]) -> float:
    ...
    return float(np.clip(np.std(arr) / (np.mean(arr)+1e-9) * 100.0, 0.0, 10.0))  # % capped
...
def bar_gradient(df: pd.DataFrame, label_col: str, value_col: str, title: str):
    ...
    return (alt.Chart(d).mark_bar()
            .encode(x=alt.X(f"{label_col}:N", sort='-y', title=None),
...
def heat_rank(df: pd.DataFrame, metric: str = "Win p %"):
    ...
    d = d.sort_values(["Race", metric], ascending=[True, False])
    d["Horse Short"] = d["Horse"].str.slice(0, 18)
    return (alt.Chart(d).mark_rect()
            .encode(x=alt.X("Horse Short:N", sort=None, title=None),
                    y=alt.Y("Race:N", sort=None, title=None),
                    color=metric_scale(metric, invert=False),
                    tooltip=["Race","Horse","Odds",metric,"EdgeBrain Score","EV"])
            .properties(height=max(36, 28 * d["Race"].nunique())))
...
def _history_for_race(race_df: pd.DataFrame) -> pd.DataFrame:
    """
    Build a long DataFrame of historical odds for all runners in a race
    using st.session_state.price_hist (we store (ts, odds) per key).
    EV_t is computed using current Fair p as an anchor.
    """
    rows = []
    for _, r in race_df.iterrows():
        ...
        for ts, odds in dq:
            odds = float(odds)
            ev_t = fair_p * odds - (1 - fair_p)  # per-£ EV proxy
            rows.append({
                "ts": pd.to_datetime(ts, unit="s", utc=True),
                "t_rel_s": ts - dq[0][0],  # seconds since first seen
                "Odds_t": odds,
                "EV_t": ev_t,
                "Horse": r["Horse"],
                "Course": r["Course"],
...
def chart_ev_heatstrip(race_df: pd.DataFrame, top_n: int = 6):
    """
    Heat 'ribbons' per horse across time: x = time bins, y = horse, color = EV_t (green→red).
    """
    hist = _history_for_race(race_df)
    if hist.empty:
        return None
    metric = "Win p %" if "Win p %" in race_df.columns else "EdgeBrain Score"
    keep = race_df.sort_values(metric, ascending=False).head(top_n)["Horse"].tolist()
    d = hist[hist["Horse"].isin(keep)].copy()
    d["t_bin"] = (d["t_rel_s"] // 5).astype(int)  # 5s bins
    ...
    return (
        ...
        .encode(
            ...
            tooltip=[alt.Tooltip("Horse:N"), alt.Tooltip("t_bin:Q", title="t (×5s)"), alt.Tooltip("EV_t:Q", format=".2f")]
        )
        .properties(height=max(180, 28 * len(keep)))
    )

def chart_odds_trend(race_df: pd.DataFrame, top_n: int = 6):
    """
    Multi‑line odds over time for top runners.
    Line color encodes **final EV sign** (green if EV_last≥0, red otherwise).
    """
    hist = _history_for_race(race_df)
    if hist.empty:
        return None
    metric = "Win p %" if "Win p %" in race_df.columns else "EdgeBrain Score"
    keep_df = race_df.sort_values(metric, ascending=False).head(top_n)
    keep = keep_df["Horse"].tolist()
    d = hist[hist["Horse"].isin(keep)].copy()
    ...
    return (
        ...
        .encode(
            x=alt.X("t_rel_s:Q", title="Seconds (relative)"),
            y=alt.Y("Odds_t:Q", title="Odds"),
            detail="Horse:N",
            color=alt.Color("EV_pos:N", scale=color_scale, legend=alt.Legend(title="Latest EV")),
            tooltip=["Horse","t_rel_s","Odds_t","EV_t"]
        )
...
def edgebrain_enhance(df: pd.DataFrame) -> pd.DataFrame:
    ...
    w_base, w_mom, w_press, w_sp, w_value, w_liq, w_spread, w_vol = 0.50, 0.18, 0.10, 0.06, 0.08, 0.05, -0.04, -0.03
    out["EdgeBrain Score"] = (
        w_base   * out["Base p %"] +
        w_mom    * np.clip(out["Momentum"] + 1.5, -6.0, 6.0) +
        w_press  * out["Pressure"] +
        w_sp     * np.clip(-out["SP Deviation %"]/2.0, -6.0, 6.0) +
        w_value  * value_boost +
        w_liq    * out["LiqScore"] * 10.0 +
        w_spread * out["Spread %"] +
        w_vol    * out["Vol %"]
    ).clip(0, 100).round(1)
...
if sel == "Overview":
    ...
    if df_live.empty:
        ...
        # Compact Race Tape for the next race (optional, auto-picks earliest time)
        try:
            ...
            mini_heat = chart_ev_heatstrip(mini, top_n=5)
    ...
    if df.empty:
        ...
        if filt.empty:
            ...
            st.dataframe(
                ...
                use_container_width=True, column_config=_format_cols(filt)
            )
    ...
    if df.empty:
        ...
        if filt.empty:
            ...
            # Scatter EV vs Score (green good → red bad)
    ...
    if df.empty:
        ...
        # Optional heatmap of upcoming races by Win p %
        ...
        if not race:
            ...
            # --- Race Tape (EV ribbons + Odds trend) ---
            with st.expander("📼 Race Tape — EV heat‑ribbons & odds trend"):
                ...
                heat = chart_ev_heatstrip(rd, top_n=6)
                if heat: st.altair_chart(heat, use_container_width=True)
                trend = chart_odds_trend(rd, top_n=6)
    ...
    if df.empty:
        ...
        st.dataframe(
            ...
            use_container_width=True, column_config=_format_cols(df)
        )
        st.markdown("---")
