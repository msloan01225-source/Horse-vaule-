...
# + Slick UI theme (deep-navy + electric-purple), polished home page & visuals
...
# ------------------ THEME ------------------
# Brand color (one color everywhere): change this to re-skin instantly
BRAND = "#7C3AED"     # Electric Purple
BRAND_SOFT = "rgba(124,58,237,0.15)"
ACCENT = "#22D3EE"    # Cyan accent for chips/links
BG = "#0B0F14"        # Deep navy
CARD_BG = "#0F1621"   # Card surface
TEXT = "#E8ECF1"      # Main text
TEXT_MUTED = "#9AA6B2" # Muted text
SUCCESS = "#22c55e"
WARN = "#f59e0b"
DANGER = "#ef4444"
...
.stSlider [data-baseweb="slider"] div[role="slider"] {{
  background: var(--brand) !important; border: 2px solid #a78bfa !important;
}}
...
div.card {{
  ...
  border-radius: 14px;
  padding: 16px 16px;
  box-shadow: 0 10px 28px rgba(0,0,0,0.35), inset 0 0 0 1px rgba(124,58,237,0.06);
}}

div.kpi {{
  display: flex; flex-direction: column; gap: 8px;
}}
div.kpi .label {{ color: var(--muted); font-size: 0.9rem; }}
...
.risk-chip {{
  padding:2px 8px;border-radius:8px;font-size:12px;font-weight:700;display:inline-block;
}}
.risk-ok {{ background: rgba(34,197,94,0.15); color: #86efac; border:1px solid rgba(34,197,94,0.25); }}
...
with st.sidebar:
    sel = option_menu(
        ...
        styles={
            ...
            "nav": {"background-color": CARD_BG},
            "icon": {"color": BRAND, "font-size": "18px"},
            "nav-link": {"font-size": "14px", "text-align": "left", "margin":"0px", "--hover-color": "rgba(124,58,237,0.15)"},
            "nav-link-selected": {"background-color": BRAND_SOFT, "color": TEXT},
        }
    )
    st.markdown("---")
...
def _hero(title: str, subtitle: str, tz_label: str):
    ...
    <div class="card" style="padding:22px 22px; margin-bottom: 10px; background:
      radial-gradient(1200px 400px at -10% -50%, rgba(124,58,237,0.18), transparent 60%),
...
def _risk_chip(risk:str)->str:
    if risk == "✅":
        return '<span class="risk-chip risk-ok">Low</span>'
    if risk == "⚠️":
        return '<span class="risk-chip risk-mid">Medium</span>'
...
def _plot_top_bar(series: pd.Series, title: str):
    ...
    fig.update_traces(marker=dict(line=dict(color="rgba(255,255,255,0.12)", width=1), opacity=0.95))
...
if sel == "Overview":
    ...
    with c1: _kpi("Countries Today", int(df_live["Country"].nunique()) if not df_live.empty else 0)
    with c2: _kpi("Courses Today", int(df_live["Course"].nunique()) if not df_live.empty else 0)
    with c3: _kpi("Runners Today", int(len(df_live)) if not df_live.empty else 0)
    with c4: _kpi("With PLACE Market", int(df_live["Odds_Place_EX"].notna().sum()) if "Odds_Place_EX" in df_live else 0)

    if not df_live.empty:
        # “What’s live next” strip
        st.markdown('<div class="card"><h4 style="margin:0 0 8px 0">Up Next</h4>', unsafe_allow_html=True)
        upcoming = df_live.copy()
        upcoming["mins"] = upcoming["MinsToOff"]
        upcoming = upcoming[upcoming["mins"] > -5].sort_values("mins").head(5)  # next 5
        if not upcoming.empty:
            cols = st.columns(len(upcoming))
            for i, (_, row) in enumerate(upcoming.iterrows()):
                with cols[i]:
                    st.markdown(f"""
                    <div style="padding:8px;border-radius:12px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06)">
                      <div style="color:var(--muted); font-size:12px;">{row['Country']} • {row['Course']}</div>
                      <div style="font-weight:700; font-size:18px; color:var(--text)">{row['Time']}</div>
                      <div style="color:var(--muted); font-size:12px;">{row['Horse']}</div>
                      <div style="margin-top:6px;">
                        <span class="badge">Odds {row['Odds'] if not np.isnan(row['Odds']) else '-'}</span>
        ...
        if view == "📊 Charts":
            ...
            st.write(
                show.to_html(escape=False, index=False).replace("<table", "<table style='width:100%;border-collapse:separate;border-spacing:0 6px;color:var(--text)'").replace(
                    "<th", "<th style='text-align:left;color:var(--muted);padding:8px 6px;border-bottom:1px solid rgba(255,255,255,0.06)'").replace(
                    "<td", "<td style='padding:6px 6px;border-bottom:1px solid rgba(255,255,255,0.03)'"),
                unsafe_allow_html=True
            )
            st.markdown('</div>', unsafe_allow_html=True)
