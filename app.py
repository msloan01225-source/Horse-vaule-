...
def fetch_racecards(day="today"):
    ...
    params = {"day": day}  # Correct: use 'day' not 'date'
    try:
        resp = requests.get(url, auth=HTTPBasicAuth(USERNAME, PASSWORD), params=params, timeout=10)
...
def fetch_mock_data():
    # Simple fallback data so UI remains functional if API fails
    return {
        "meetings": [{
...
def fetch_racecards_with_fallback():
    for day in ["today", "tomorrow"]:
        ...
        if not df.empty:
            return df, day
    return process_data(fetch_mock_data()), "mock"
...
if df.empty:
    ...
    st.success(f"Displaying races for **{label}**")
    st.write(f"Last updated: {datetime.utcnow():%Y‑%m‑%d %H:%M UTC}")
