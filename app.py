# edgebet_app.py — EdgeBet Pro
# -------------------------------------------------
# A polished, glitch‑free Streamlit app with an "out-of-this-world" EdgeBrain.
# -------------------------------------------------
...
def apply_theme():
    ...
    <style>
      ...
      .css-1dp5vir, .stMetric, .stTable, .stDataFrame {{ background:{card} !important; border-radius:10px; }}
...
if "price_hist" not in st.session_state:
    # key: f"{marketId}:{selectionId}" -> deque of (ts, odds)
    st.session_state.price_hist = defaultdict(lambda: deque(maxlen=60))
if "bankroll" not in st.session_state:
    st.session_state.bankroll = 1000.0  # default demo bankroll
...
def _format_cols(df: pd.DataFrame) -> dict:
    ...
    for col in df.columns:
        ...
        if col in ("Odds","Fair Odds","SP (proj)","EV"): fmt[col] = "{:.2f}"
...
def _best_prices_sizes(rb: dict):
    ...
    return best_back, best_lay, back_sz, lay_sz, proj
...
def bf_list_win_markets(day="today"):
    ...
    params = {
        "filter": {
            "eventTypeIds": ["7"],
            "marketCountries": ["GB"],  # keep focused/fast; can extend later
            "marketTypeCodes": ["WIN"],
...
def _overround_and_fair_probs(df: pd.DataFrame):
    # per race key (Course + Time) compute overround and fair probs
    df = df.copy()
...
def build_live_df_betfair(day="today") -> pd.DataFrame:
    ...
    for mb in books:
        mid = mb.get("marketId")
        latency_map[mid] = mb.get("totalMatched", 0.0)
    ...
    # Value columns (yours) for continuity
...
def _momentum(series_odds: list[float]) -> float:
    """Return momentum score: negative slope (odds shortening) -> positive signal."""
    if len(series_odds) < 3: return 0.0
    y = np.array(series_odds[-10:])  # last up to 10 points
    x = np.arange(len(y))
    # regress odds -> probability to linearize impact
    p = 1.0 / np.clip(y, 1.01, None)
    A = np.vstack([x, np.ones_like(x)]).T
    slope, _ = np.linalg.lstsq(A, p, rcond=None)[0]
    # scale to percentage points: typical slope range ~[-0.02,0.02]
    return float(np.clip(slope * 100.0, -5.0, 5.0))

def _pressure(back_sz: float, lay_sz: float) -> float:
    """Book pressure: more back liquidity vs lay -> positive."""
    total = max(back_sz + lay_sz, 1e-6)
    ratio = (back_sz - lay_sz) / total  # -1..1
    return float(np.clip(ratio * 10.0, -5.0, 5.0))  # scale to -5..5

def edgebrain_enhance(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty: return df
    out = df.copy()

    # Momentum via session sparkline
    keys = (out["MarketId"].astype(str) + ":" + out["SelectionId"].astype(str)).tolist()
    m_scores = []
    for k in keys:
        s = _sparkline_series(k, 30)
        m_scores.append(_momentum(s))
    out["Momentum"] = m_scores  # in %-pts

    # Pressure from market depth
    out["Pressure"] = out.apply(lambda r: _pressure(r["Back Size"], r["Lay Size"]), axis=1)

    # Anchor to SP projection if available (deviation)
    out["SP Deviation %"] = np.where(out["SP (proj)"].notna(),
                                     (out["SP (proj)"] - out["Odds"]) / out["Odds"] * 100.0, 0.0).round(1)

    # Fair probability (de‑vig) is already in out["Fair p"]
    out["Base p %"] = (out["Fair p"] * 100.0).round(2)

    # Composite EdgeBrain Score (0–100)
    # weights chosen to balance market signal with microstructure
    w_base, w_mom, w_press, w_sp, w_value = 0.56, 0.16, 0.10, 0.08, 0.10
    value_boost = ((1.0/np.clip(out["Odds"],1.01,None)) - out["Fair p"]) * 100.0  # positive if price > fair
    out["EdgeBrain Score"] = (
        w_base * out["Base p %"] +
        w_mom  * np.clip(out["Momentum"] + 2.0, -5.0, 5.0) +  # slight prior to shortening
    ...
    # EV, Kelly
    p = out["Fair p"].clip(0.001, 0.999)               # model prob after de-vig
    q = 1 - p
    b = out["Odds"] - 1.0
    out["EV"] = (p * (out["Odds"]) - q).round(2)       # expected profit per £1 stake
    kelly = (b * p - q) / b
    out["Kelly %"] = (np.clip(kelly, 0, 1.0) * 100.0).round(1)

    # Suggested stake with cap
    bankroll = max(float(st.session_state.bankroll), 0.0)
    cap = bankroll * 0.02  # 2% cap default
...
def dutch_stakes(odds: list[float], total_stake: float, method: str = "equal_return"):
    ...
    if method == "equal_return":
        inv = 1.0 / (odds - 1.0)
        w = inv / inv.sum()
        stakes = total_stake * w
    else:  # proportional to EdgeBrain score (caller passes weights)
        raise ValueError("Unsupported method")
    return np.round(stakes, 2)

# ------------------ SIDEBAR NAV ------------------
with st.sidebar:
    sel = option_menu(
        "🏇 EdgeBet Pro",
        ["Overview", "Value Finder", "Dutching Lab", "EdgeBrain", "Debug"],
        icons=["house", "fire", "intersect", "robot", "bug"],
        default_index=0
    )
...
if sel == "Overview":
    ...
    if df.empty:
        ...
        with c3:
            min_ev = st.slider("Min EV", -0.50, 1.00, 0.05, 0.01)
        with c4:
            min_score = st.slider("Min EdgeBrain", 0, 100, 22, 1)
        ...
        if filt.empty:
            ...
            def sparkline_cell(key):
                ...
                # draw using a very small plotly-ish SVG (no external deps) -> simple bar chars
            ...
            cols = ["Badges","Risk","Horse","Course","Time","Odds","Fair Odds","EV","Kelly %","Stake £",
    ...
    if df.empty:
        ...
        subset = df[(df["Course"] + " | " + df["Time"]) == race].copy()
        subset = subset.sort_values(["EdgeBrain Score","EV"], ascending=[False, False])
        st.write("Select runners to dutch:")
        selected = st.multiselect("Runners", subset["Horse"] + " @ " + subset["Odds"].astype(str),
                                  default=(subset["Horse"] + " @ " + subset["Odds"].astype(str)).head(3).tolist())
        if selected:
    ...
    if df.empty:
        ...
        st.dataframe(
            df[[
                "Country","Course","Time","Horse","Odds","Fair Odds","EV","Kelly %","Stake £",
