    ...
    with st.spinner("Loading race data..."):
        ...
        if df.empty:
            ...
            if df.empty:
                ...
                # Select columns to display (including new stats)
                display_cols = ["Horse", "Jockey", "Trainer", "Form", "Official Rating", "Exchange Odds", "Value Score", "Race URL"]

                # Format the URL column as clickable links
                def make_clickable(url):
                    return f"[Link]({url})"

                df["Race URL"] = df["Race URL"].apply(make_clickable)

                st.write(df[display_cols].reset_index(drop=True).style.format({
                    "Value Score": "{:.3f}",
                    "Exchange Odds": "{:.2f}"
