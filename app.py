# edgebet_app.py — EdgeBet Ultra
# ---------------------------------------------------------------------------
# Slick design system + stronger value engine + full analysis workflow.
# What’s new vs prior version:
#   • Design System: CSS variables, consistent palette, Altair theme, green=best.
#   • De‑vig: Power normalization (fits margin shape better than proportional).
#   • Restored "Horse Racing" + upgraded with consistent charts.
#   • New "Race Hub": one-race deep-dive (market health, rankings, sparkline).
#   • Bet Tracker: add picks, expected return & Kelly, bankroll-aware.
#   • Kept: EdgeBrain v2, Value Finder, Dutching, Ladder, EW EV, GB/IE toggle,
#           Alerts, Mock mode, sticky filters, CSV, autorefresh.
# ---------------------------------------------------------------------------
...
def apply_design_system():
    ...
    if is_dark:
        bg, sbg, text, card, axis = PALETTE["ink_3"], PALETTE["ink_2"], PALETTE["text_d"], "#143f37", "#bfeee8"
    else:
        bg, sbg, text, card, axis = PALETTE["paper"], "#F0F2F1", PALETTE["text_l"], PALETTE["paper_2"], "#2b6d63"

    st.markdown(f"""
    <style>
      :root {{
        --c-primary: {PALETTE["teal"]};
        --c-primary-strong: {PALETTE["teal_700"]};
        --c-accent: {PALETTE["green"]};
      ...
      h1, h2, h3, h4, h5, h6 {{ color: var(--c-primary); text-shadow: 0 0 8px rgba(0,191,166,.3); }}
      ...
      .badge-ev {{ background:#e8fff7; color:#00664f; border:1px solid var(--c-primary); }}
      ...
      .badge-risk {{ background:#ffe8ef; color:#9a0036; border:1px solid #ff7aa5; }}
    </style>
    """, unsafe_allow_html=True)

apply_design_system()

# Consistent Altair theming (green max, teal base)
def _altair_theme():
    return {
        "config": {
            "view": {"stroke": "transparent"},
            "axis": {"labelColor": "var(--c-text)", "titleColor": "var(--c-text)", "domainColor":"var(--c-axis)", "tickColor":"var(--c-axis)"},
...
# Fallback Racing API (names only; mock odds)
...
def devig_power(odds: pd.Series) -> pd.Series:
    ...
    This flexibly fits margin shape (often better than proportional).
    """
    ...
    # find alpha via bisection on [0.5, 2.0]
    lo, hi = 0.5, 2.0
    for _ in range(40):
        mid = 0.5 * (lo + hi)
        s = sum_p(mid)
...
# ------------------ EDGE BRAIN v2 (unchanged maths, now uses Fair p) ------------------
...
def bar_green_max(df: pd.DataFrame, label_col: str, value_col: str, title: str):
    ...
    return alt.Chart(d).mark_bar().encode(
        ...
        color=alt.condition(alt.datum.is_max, alt.value(PALETTE["green"]), alt.value(PALETTE["teal"])),
        tooltip=[label_col, value_col]
    ).properties(height=320)

def sparkline_chart(series: list[float]):
    if not series: return alt.Chart(pd.DataFrame({"x":[0], "y":[0]})).mark_line().encode(x="x", y="y")
    df = pd.DataFrame({"x": list(range(len(series))), "y": series})
    mx = float(max(series)); mi = float(min(series))
    col = PALETTE["green"] if (len(series)>1 and series[-1]<=series[0]) else PALETTE["amber"]
    return alt.Chart(df).mark_line(color=col, strokeWidth=2).encode(x="x:Q", y="y:Q").properties(height=60)
...
def get_df(day="today") -> pd.DataFrame:
    ...
    # Legacy engineered metrics (for Horse Racing classic view)
    ...
    # Fair probs (power devig) + EdgeBrain v2 + EW EV
...
if sel == "Overview":
    ...
    if df.empty:
        ...
        if filt.empty:
            ...
            def sparkline_cell(key):
                ...
                # tiny ascii sparkline: green when shortening, amber otherwise (consistent palette)
                ...
                color = PALETTE["green"] if good else PALETTE["amber"]
    ...
    # If we have a live df, show a selector to add
