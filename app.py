...
# ------------------ HTML TABLE RENDERER ------------------
def render_html_table(styled_or_df, height: int = 600):
    """Render a (Styled) DataFrame as fully controlled HTML so colors always show."""
    try:
        html_table = styled_or_df.to_html(escape=False)
    except AttributeError:
        html_table = styled_or_df.to_html(escape=False, index=False)

    is_dark = st.session_state.theme_mode == "dark"
    header_color = "#00bfa6" if is_dark else "#00796b"
    ...
    text_color = "#f2f2f2" if is_dark else "#111"

    html = f"""
    <div class="edge-table-wrap" style="max-height:{height}px; overflow:auto; border:1px solid {header_color}; border-radius:6px;">
      <style>
...
# ------------------ EDGE BRAIN ------------------
    ...
    if df.empty:
        ...
        if view == "Both":
            ...
            styled_df = df[["Country","Course","Time","Horse","Odds","EdgeBrain Win %","Risk_v1","Source"]]
            render_html_table(styled_df.style)
        else:
            styled_df = df[["Country","Course","Time","Horse","Odds","EdgeBrain+ Win %","Risk+","Source"]]
            render_html_table(styled_df.style)
