# edgebet_app.py — EdgeBet Ultra (Matrix Edition, EdgeBrain v3)
# ---------------------------------------------------------------------------
# What's new:
#  • EdgeBrain v3 "Predictor" with race search → instant Top 3 (probabilities)
#  • Stronger scoring (fair prob + momentum + pressure + SP anchor + liquidity + spread + vol)
#  • Explainability drawer per runner (weight breakdown)
#  • Removed: Dutching Lab, Race Hub
#  • Kept: Matrix theme + rain, Overview, Horse Racing (classic), Value Finder, EdgeBrain, Bet Tracker
# ---------------------------------------------------------------------------
...
if "price_hist" not in st.session_state:
    st.session_state.price_hist = defaultdict(lambda: deque(maxlen=90))  # a bit longer for vol
...
# Fallback Racing API (names + mock odds)
...
def _spread(best_back, best_lay) -> float:
    ...
    return float(np.clip((best_lay - best_back) / (mid + 1e-9) * 100.0, 0.0, 12.0))  # % cap
...
def edgebrain_enhance(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ensure fair p is there
    out["Fair p"] = out["Fair p"].clip(0.001, 0.999)
    out["Base p %"] = (out["Fair p"] * 100.0).round(2)

    # Value delta (exchange vs fair)
    value_boost = ((1.0/np.clip(out["Odds"],1.01,None)) - out["Fair p"]) * 100.0

    # v3: more expressive + risk-awareness
    # weights tuned heuristically; Liq and Spread downweight noisy markets; Vol penalises unstable moves.
    ...
    out["EdgeBrain Score"] = (
        ...
        w_value  * value_boost +
        w_liq    * out["LiqScore"] * 10.0 +          # scale 0..40
        w_spread * out["Spread %"] +                  # wider spread -> worse
        w_vol    * out["Vol %"]                       # higher vol -> worse
    ).clip(0, 100).round(1)
...
def predict_top3(race_df: pd.DataFrame) -> pd.DataFrame:
    """
    Turn v3 features into a softmax win probability + Top3.
    We calibrate a WinScore then softmax across runners.
    """
    if race_df.empty: return race_df
    df = race_df.copy()

    # Build WinScore (transparent & monotonic-ish)
    # Start from devig fair prob (already good baseline)
    score = (
        1.10 * df["Fair p"] +                              # baseline
        0.015 * df["Momentum"] +                           # shortening trend
        0.010 * df["Pressure"] +                           # back side dominant
        0.006 * (-df["SP Deviation %"]) +                  # price below SP proj
        0.002 * (df["LiqScore"] * 10.0) +                  # liquidity health
        -0.002 * df["Spread %"] +                          # tighter = better
        -0.0015 * df["Vol %"] +                            # lower vol = better
        0.004 * np.clip((1.0/np.clip(df["Odds"],1.01,None)) - df["Fair p"], -0.2, 0.2) * 100.0  # value delta
    )

    # Normalise to probabilities via softmax (temperature for sharpness)
    temp = 0.85
    logits = score / max(temp, 1e-6)
    # subtract max for stability
    logits = logits - logits.max()
    expv = np.exp(np.clip(logits, -50, 50))
    winp = expv / (expv.sum() + 1e-9)
...
if sel == "Overview":
    ...
    if df.empty:
        ...
        if filt.empty:
            ...
            def sparkline_cell(key):
                series = _sparkline_series(key, 60)
            ...
            cols = ["Badges","Risk","Horse","Course","Time","Odds","Fair Odds","EV","EW EV","Kelly %","Stake £",
                    "EdgeBrain Score","Momentum","Pressure","SP (proj)","Drift","Back Size","Lay Size","Spread %","Vol %","Overround","Source"]
    ...
    if df.empty:
        ...
        if q:
            ql = q.strip().lower()
            choices = [r for r in races if ql in r.lower()]
        else:
            choices = races
        ...
        if not race:
            ...
            # Compute Top 3
            picks = predict_top3(rd)
            top3 = picks.head(3).copy()
            ...
            # Explainability for each of the Top 3
            st.markdown("### Explain")
            for _, r in top3.iterrows():
                with st.expander(f"{r['Horse']} — Win {r['Win p %']:.1f}% | Odds {r['Odds']:.2f} | EB {r['EdgeBrain Score']:.1f}"):
